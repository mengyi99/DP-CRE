...
class data_sampler(object):
    ...
    def _read_data(self, file):
        ...
        for relation in tqdm(data.keys(), desc="Load data"):
            ...
            for i, sample in enumerate(rel_samples):
                ...
                'tokens': self.tokenizer.encode(' '.join(sample['tokens'])),
                'string': '[RelData] ' + sample['relation'] + ' ' + ' '.join(sample['tokens'])
                }
    ...
    def _read_relations(self, file):
        ...
        :return: a tuple containing a list of relations and a mapping from relations to their ids
        '''
        with open(file, 'r', encoding='utf-8') as f:
            id2rel = json.load(f)
        rel2id = {x: i for i, x in enumerate(id2rel)}
